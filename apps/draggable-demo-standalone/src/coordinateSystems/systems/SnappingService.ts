  ...
  private snapToObjects(
    ...
    for (const obj of allObjects) {
      ...
      const snapCandidates = [
        // Align left edges
        new Vector2(obj.physicObject?.position.x || 0, position.y),
        // Align right edges
        new Vector2((obj.physicObject?.position.x || 0) + (obj.physicObject?.Width || 0) - (draggedObject.physicObject?.Width || 0), position.y),
        // Align top edges
        new Vector2(position.x, obj.physicObject?.position.y || 0),
        // Align bottom edges
        new Vector2(position.x, (obj.physicObject?.position.y || 0) + (obj.physicObject?.Height || 0) - (draggedObject.physicObject?.Height || 0)),
        // Align centers horizontally
        new Vector2(
          (obj.physicObject?.position.x || 0) + (obj.physicObject?.Width || 0) / 2 - (draggedObject.physicObject?.Width || 0) / 2,
          position.y
        ),
        // Align centers vertically
        new Vector2(
          position.x,
          (obj.physicObject?.position.y || 0) + (obj.physicObject?.Height || 0) / 2 - (draggedObject.physicObject?.Height || 0) / 2
        )
      ];
